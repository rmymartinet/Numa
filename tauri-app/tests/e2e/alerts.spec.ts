...
test.describe('Alerts System E2E Tests', () => {
  ...
  test('should display alerts panel when button is clicked', async ({ page }) => {
  ...
  test('should acknowledge and resolve alerts', async ({ page }) => {
    ...
    if (alertCount > 0) {
      ...
      const acknowledgeButton = page.locator('button:has-text("Acknowledger")').first();
  ...
  test('should display rules tab', async ({ page }) => {
    ...
    await expect(page.locator('text=Règles d\'Alerte Configurées')).toBeVisible();
  ...
  test('should show alert severity indicators', async ({ page }) => {
    ...
    const severityIndicators = page.locator('[class*="px-2 py-1 rounded text-xs font-medium"]');
  ...
  test('should handle alert actions', async ({ page }) => {
    ...
    const actionButtons = page.locator('button:has-text("Acknowledger"), button:has-text("Résoudre")');
